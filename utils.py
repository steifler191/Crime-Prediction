...
def load_data_GAT(bs):
    ...
    idx = np.array(idx_features_labels[:num_reg, 0], dtype=np.int)
    idx_map = {j: i for i, j in enumerate(idx)}
    edges_unordered = np.genfromtxt("tem_gat_adj.txt", dtype=np.int)
    edges = np.array(list(map(idx_map.get, edges_unordered.flatten())), dtype=np.int).reshape(edges_unordered.shape)
...
def gen_com_adj_matrix(target_region):
    ...
    edges_unordered = np.genfromtxt("data/com_adjacency.txt", dtype=np.int)
...
def gen_gat_adj_file(target_city, target_region):
    neighbors = gen_neighbor_index_zero(target_region, target_city)
    print("neighbours of target_region = " + str(neighbors))
